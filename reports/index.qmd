...
assertthat::assert_that(assertthat::is.number(tree_selection))
...
pretty_names <- tribble(
  ...
  "all rbcl_short", "*rbcL* (short)", 1,
  "all rbcl", "*rbcL*", 2,
  "all trnlf", "*trnL-F*", 3,
  "all rbcl_trnlf", "*rbcL* + *trnL-F*", 4,
...
Classification followed PPG I, with updates for some recently published or revised genera; families with fewer than two samples excluded from analysis. 
...
For the tree test, we first aligned the DNA sequences with MAFFT. 
The *trnL-F* region includes intergeneic spacers and its evolutionary rate is too high to reliably align it across all species in our study, which spans all ferns, so we first conducted alignment within each family, then merged the family-level alignments into a single alignment using the MAFFT `--merge` argument [@Nitta2022b].
We then inferred a maximum-likelihood phylogeny using IQTREE [@Nguyen2015].
...
We calculated the mean intraspecific percent identity (pident) from the BLAST results and used this as the species level cutoff value.
...
**Fig. 2.** Maximum-likelihood *rbcL* + *trnL-F* phylogeny overlaid with results of the DNA barcode tests and hybrid status from the *rbcL* + *trnL-F* dataset.
